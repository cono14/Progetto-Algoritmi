...
public class LXInomove implements CXPlayer {
    private Random rand;
    ...
    public int eval(CXBoard board, int col) {
        ...
        for (int k = 1; k<= board.X && col - k >= 0 && (board.cellState(last_row, col - k) == s
                || board.cellState(last_row, col - k) == CXCellState.FREE); k++) {
            if (board.cellState(last_row, col - k) == s)
                n++;
        } // backward check
        for (int k = 1; k<= board.X && col + k < board.N && (board.cellState(last_row, col + k) == s
                || board.cellState(last_row, col + k) == CXCellState.FREE); k++) {
            if (board.cellState(last_row, col + k) == s)
                n++;
        } // forward check
        ...
        // Vertical check
        n = 1;
        for (int k = 1; k<= board.X && last_row + k < board.M && (board.cellState(last_row + k, col) == s
        ...
        // Diagonal check
        n = 1;
        for (int k = 1; k<= board.X && last_row - k >= 0 && col - k >= 0
        ...
        for (int k = 1; k<= board.X && last_row + k < board.M && col + k < board.N
                && (board.cellState(last_row + k, col + k) == s
                        || board.cellState(last_row + k, col + k) == CXCellState.FREE); k++) {
            if (board.cellState(last_row + k, col + k) == s)
                n++;
        ...
        // Anti-diagonal check
        n = 1;
        for (int k = 1; k<= board.X && last_row - k >= 0 && col + k < board.N
                && (board.cellState(last_row - k, col + k) == s || board.cellState(last_row - k, col
                        + k) == CXCellState.FREE); k++) {
            if (board.cellState(last_row - k, col + k) == s)
                n++;
        } // backward check
        for (int k = 1; k<= board.X && last_row + k < board.M && col - k >= 0
                && (board.cellState(last_row + k, col - k) == s || board.cellState(last_row + k, col
                        - k) == CXCellState.FREE); k++) {
            if (board.cellState(last_row + k, col - k) == s)
                n++;
    ...
    public int alphabeta(CXBoard board, int depth, int alpha, int beta, boolean maximizingPlayer) {
        ...
        // Massimizzare la valutazione per il giocatore corrente Best
        if (maximizingPlayer) {
    ...
    private int alphabetacol(CXBoard B, int depth) {
        ...
        for (int i : B.getAvailableColumns()) {
            ...
            if (B.gameState() == myWin) {
                System.out.println("LXX the best"); // serve a qualcosa
                return i;
            }
        ...
        for (int k = 1; k < B.getAvailableColumns().length; k++) {
            if (B.gameState() == CXGameState.OPEN) {
                ...
                if (B.gameState() == yourWin) {
                    System.out.println("LXX Lose Avoided");
