...
public class LXMARGIN implements CXPlayer {
    ...
    public int eval(CXBoard board, int col) {
        ...
        for (int j = k; j <= board.X && col - j >= 0 && (board.cellState(last_row, col - j) == s
            ...
            // poi si continua a guardare, sempre nel margine, per pezzi con collegati ma
            // vicini
            margin++;
            if (board.cellState(last_row, col - j) == s)
                notconnected++;
        }
        k = 1;
        // backward check
        ...
        k = 1;
        while (k <= board.X && last_row + k < board.M && col + k < board.N
        ...
        margin = 0;
        k = 1;
        while (k <= board.X && last_row - k >= 0 && col + k < board.N
                && (board.cellState(last_row - k, col + k) == s)) {
            n++;
            k++;
        }
        ...
        k = 1;
        while (k <= board.X && last_row + k < board.M && col - k >= 0
                && (board.cellState(last_row + k, col - k) == s)) {
            n++;
            k++;
        }
